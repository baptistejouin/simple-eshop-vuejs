<template>
	<Teleport to="#app">
		<div :class="active ? 'pointer-events-auto' : 'pointer-events-none'" :aria-hidden="!active" class="fixed inset-0 z-10">
			<Transition name="cart-transition" enter-active-class="translate-x-full" leave-active-class="translate-x-full">
				<aside v-show="active" class="transition-transform duration-50 ease-out w-full h-full bg-black ml-auto text-white py-8 sm:max-w-[450px]">
					<h2 class="text-xl font-bold pb-8 px-8 border-b border-neutral-700">Shopping cart</h2>
					<div class="px-8 pt-8">
						<ProductSidebar />
					</div>
				</aside>
			...
			<Transition name="bg-transition" enter-active-class="opacity-0" leave-active-class="opacity-0">
				<div v-show="active" class="fixed inset-0 bg-black bg-opacity-25 -z-10 opacity-100 transition-opacity duration-25" @click="$emit('toggleShopCart')"></div>
			</Transition>
		</div>
	</Teleport>
</template>
